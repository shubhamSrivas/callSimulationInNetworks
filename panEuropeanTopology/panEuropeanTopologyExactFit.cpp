...
class WDM : public Graph
{
    public:
        ...
        int exact_fit(int source,int destination,int slots_required,int request_number,vector<int> &slot_arr);
...
void Graph::construct_graph()
{
    ...
    edges = 41; //No. of edges. 
       
    cout<<edges;
    this->addEdge(0,1,462);
    this->addEdge(0,3,1067);
    this->addEdge(1,2,690);
    this->addEdge(2,3,540);
    this->addEdge(2,5,514);
    this->addEdge(3,4,259);
    this->addEdge(3,10,552);
    this->addEdge(4,5,393);
    this->addEdge(4,9,474);
    this->addEdge(5,6,594);
    this->addEdge(5,8,600);
    this->addEdge(5,11,747);
    this->addEdge(6,7,570);
    this->addEdge(6,13,796);
    this->addEdge(7,8,218);
    this->addEdge(7,14,327);
    this->addEdge(8,9,271);
    this->addEdge(9,10,592);
    this->addEdge(9,15,456);
    this->addEdge(10,16,381);
    this->addEdge(11,12,834);
    this->addEdge(12,13,760);
    this->addEdge(14,26,720);
    this->addEdge(14,15,522);
    this->addEdge(15,16,757);
    this->addEdge(15,22,534);
    this->addEdge(16,17,540);
    this->addEdge(16,21,420);
    this->addEdge(16,20,775);
    this->addEdge(17,18,722);
    this->addEdge(18,19,623);
    this->addEdge(19,20,1213);
    this->addEdge(20,23,618);
    this->addEdge(21,22,376);
    this->addEdge(21,23,668);
    this->addEdge(22,24,400);
    this->addEdge(23,25,474);
    this->addEdge(24,25,551);
    this->addEdge(24,26,783);
    this->addEdge(25,27,1209);
    this->addEdge(26,27,1500);
}
...
int WDM::exact_fit(int src,int dest,int slots_required,int request_number,vector<int> &slot_arr){
    ...
    int last,start,mCtr=321;//variable to represent starting and last free slot number
    bool wavelength_available;//boolean to represent wavelength availability on that path
    bool slot_available[num_slots];
    vector<Edge> path=this->Dijkstra(src,dest);
    cout<<"Path: ";
    for(int i=path.size()-1;i>0;i--)
        cout<<path[i].first<<"->"<<path[i-1].first<<"("<<path[i].second<<")"<<" ";
    cout<<endl;
    for(q=0;q<this->num_wavelengths;q++){
        start=0;
        cfs=0;
        ...
        for(i=path.size()-1;i>0;i--){
            ...
            for(slot_number=0;slot_number<num_slots;slot_number++){
        ...
        for(i=0;i<num_slots;i++){
            if(slot_available[i]){
                ...
                first=i+1;
                if(cfs>=slots_required&&cfs-slots_required<mCtr){
                    mCtr=cfs-slots_required;
                    start=first;
                }
                cfs=0;
            }
        ...
        if(cfs<slots_required){
            ...
            cout<<"exactFit::wavelength "<<q<<" not available"<<endl;
            continue;
        }
        if(wavelength_available){
            last=start+slots_required-1;
            cout<<"exactFit::wavelength "<<q<<" is available "<<endl<<endl;
            for(i=path.size()-1;i>0;i--){
                origin=path[i].first;
                end=path[i-1].first;
                for(slot_number=start;slot_number<=last;slot_number++){
                    for(j=0;j<Network[origin].size();j++)
                        if(slot_arrays[slot_number][q][origin][j].first==end)
                        break;
                    slot_arr[slot_number]++;//1d array to find max used slot index
  ...
  for(call_number=1;call_number<call_end_time.size();call_number++)
  {
    if(call_end_time[call_number]==-1)//call has been serviced
    ...
    else
      {
        ...
        cout<<"EF::Call "<<call_number<<", completed."<<endl;
      }
  }

  cout<<"EF::Calls completed : "<<calls_completed<<", Calls in progress : "<<calls_in_progress<<endl;
...
int main() {
    int nodes=28,edge_density,density,waves,slots;
